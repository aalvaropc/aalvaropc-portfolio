  ...
  useEffect(() => {
    ...
    const detectLocale = async () => {
      try {
        ...
        // 4. Try geolocation only once per session, with user consent
        if (
          'geolocation' in navigator &&
          !sessionStorage.getItem('geolocation-attempted')
        ) {
          sessionStorage.setItem('geolocation-attempted', 'true')

          const userConsented = window.confirm(
            navigator.language.startsWith('es')
              ? '¿Permitir acceso a tu ubicación para detectar tu idioma preferido?'
              : 'Allow access to your location to detect your preferred language?'
          )
            ...
            async position => {
              ...
              try {
                ...
                if (response.ok && isMounted) {
                  ...
                  const detectedLocale = SPANISH_SPEAKING_COUNTRIES.includes(
                    countryCode
                  )
                    ? 'es'
                    : 'en'
