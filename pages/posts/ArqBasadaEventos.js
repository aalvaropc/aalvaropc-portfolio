...
const ArqBasadaEventos = () => {
    const { postDetail, loading, error } = usePostDetail('ArqBasadaEventos')
    const { t } = useI18n()
        ...
        <Layout title={postDetail.title}>
            <Container>
                ...
                <List ml={4} my={4}>
                    ...
                    {postDetail.meta.event && (
                        <ListItem>
                            <Meta>{t('metadata.event')}</Meta>
                            <Link href={postDetail.meta.event}>
                                {postDetail.meta.event} <ExternalLinkIcon mx="2px" />
                            </Link>
                        </ListItem>
                    )}
                    <ListItem>
                ...
                {postDetail.sections.topics && (
                    <>
                        <List ml={4} my={4}>
                            {postDetail.sections.topics.items.map((item, index) => (
                                <ListItem key={index}>
...
export default ArqBasadaEventos
