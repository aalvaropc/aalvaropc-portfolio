...
jest.mock('@chakra-ui/react', () => {
  ...
  return {
    ...
    Heading: ({ children, as = 'h1', ...allProps }) => {
      const domProps = filterDOMProps(allProps)
      return React.createElement(as, { 'data-testid': 'heading', ...domProps }, children)
    },
    Button: ({ children, _as, href, target, rel, onClick, isDisabled, ...allProps }) => {
      const domProps = filterDOMProps({ onClick, href, target, rel, disabled: isDisabled, ...allProps })
      
      if (href) {
        return <a data-testid="button" {...domProps}>{children}</a>
      }
    ...
    Link: ({ children, href, target, rel, ...allProps }) => {
      ...
      return <a data-testid="chakra-link" {...domProps}>{children}</a>
    },
    List: ({ children, ...allProps }) => {
      const domProps = filterDOMProps(allProps)
      return <ul data-testid="list" {...domProps}>{children}</ul>
    },
    ListItem: ({ children, ...allProps }) => {
      const domProps = filterDOMProps(allProps)
      return <li data-testid="list-item" {...domProps}>{children}</li>
    },
...
jest.mock('../../lib/i18nContext', () => ({
  useI18n: () => ({
    ...
    getHome: () => ({
      ...
      about: {
        title: "Sobre Mí",
        content: "Desarrollador con experiencia en Go, Python y Java. Me especializo en backend con FastAPI, Spring Boot y desarrollo de APIs robustas."
      },
      experience: {
        title: "Experiencia",
        jobs: [
      ...
      contact: {
        title: "Contacto",
        links: [
          { type: "github", url: "https://github.com/aalvaropc", label: "GitHub" },
          { type: "linkedin", url: "https://www.linkedin.com/in/aalvarop-pe/", label: "LinkedIn" },
...
describe('Home Page Integration', () => {
  it('renders complete home page with all sections', () => {
    ...
    const heroElements = screen.getAllByText(/backend-focused full stack developer/i)
